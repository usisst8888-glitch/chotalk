...
export default function ErrandTalkTab() {
  const [services, setServices] = useState<ErrandTalkService[]>([]);
  ...
  const fetchServices = async () => {
    try {
      ...
      console.error('Failed to fetch errand talk services');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert('서비스 신청이 완료되었습니다. 입금 확인 후 활성화됩니다.');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/aktalk_atok?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchServices();
      } else {
        alert('삭제에 실패했습니다.');
    ...
    <>
      <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-white">심부름톡 관리</h2>
            <p className="text-neutral-400 text-sm mt-1">아톡에서 웨톡으로 메시지를 전달하는 서비스입니다.</p>
          </div>
          <button
            onClick={() => setShowApplyModal(true)}
            className="px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-lg font-medium transition"
          >
            서비스 신청
          </button>
        </div>
      ...
      {showApplyModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-bold text-white mb-4">심부름톡 서비스 신청</h3>
            <form onSubmit={handleApply} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-neutral-800 border border-neutral-700 rounded-xl text-white focus:outline-none focus:border-teal-500"
                >
                  {SHOP_NAMES.map((name) => (
                ...
                {applyForm.shopName === '기타' && (
                  <input
                    ...
                    className="w-full mt-2 px-4 py-3 bg-neutral-800 border border-neutral-700 rounded-xl text-white placeholder-neutral-600 focus:outline-none focus:border-teal-500"
                  />
                )}
              </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-2 py-1 text-xs bg-teal-600 hover:bg-teal-500 text-white rounded transition"
                      >
                        복사
                      </button>
                    </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-neutral-800 border border-neutral-700 rounded-xl text-white placeholder-neutral-600 focus:outline-none focus:border-teal-500"
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-3 bg-teal-600 hover:bg-teal-500 disabled:bg-teal-800 text-white font-semibold rounded-xl transition"
                >
