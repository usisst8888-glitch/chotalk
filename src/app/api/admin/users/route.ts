  ...
  try {
    ...
    if (authUser.role === 'superadmin') {
      const { data: users, error } = await supabase
        .from('users')
        .select('id, username, nickname, phone, role, is_premium, slot_count, parent_id, domain, header_template, created_at')
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { userId, slotCount, role, parentId, domain, headerTemplate, isPremium } = await request.json();
